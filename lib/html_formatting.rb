  ...
  module InstanceMethods
    ...
    def format_usernames(body)
      ...
      body.gsub(/(^|\W)@(\w+)/) do |text|
        ...
        #TODO: mention all??
        # if 'all' == name
        #   #TODO: restrict users here
        #   #@mentioned = project.users.confirmed
        #   @mentioned = User.all
        #   all_mentioned = true
        #   first + content_tag(:span, '@all', :class => "mention")
        #TODO: and restrict users here
        if user = User.find_by_identity(name)
          if not all_mentioned and Comment === self
            @mentioned ||= Set.new
            @mentioned << user
          end
          first + link_to("@#{user.identity}", "/users/#{user.id}", :class => 'mention')
        else
          text
        end
      end
    ...
    def format_links(text)
      Rinku.auto_link(text) { |text| truncate(text, :length => 80) }
    end
